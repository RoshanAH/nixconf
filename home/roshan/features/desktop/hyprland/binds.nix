{lib, ...}: {
  ...
  in {
    ...
    bind =
      ...
      (map (
          n: "ALTSHIFT,${n},movetoworkspacesilent,name:${n}"
        )
        workspaces)
      ++
      # Move focus
      ...
      # Move windows
      (lib.mapAttrsToList (
          key: direction: "ALTCONTROL,${key},swapwindow,${direction}"
        )
        directions);
    binde =
      # Resizing
      lib.mapAttrsToList (
        key: direction: "ALTSHIFT,${key},resizeactive,${pixelDelta.${direction}}"
      )
      directions;
  };
}
